...
func TestBuildScript_SimpleScript(t *testing.T) {
	bs := NewBuildScript("/src", "", "", false, "/root/.cache/zig")

	script := bs.Script()

	assert.Contains(t, script, "#!/bin/sh")
	assert.Contains(t, script, "set -e")
	assert.Contains(t, script, "zig build --summary all")
...
func TestBuildScript_VerboseScript(t *testing.T) {
	bs := NewBuildScript("/src", "ReleaseSafe", "", true, "/root/.cache/zig")

	script := bs.Script()

	assert.Contains(t, script, "#!/bin/sh")
	assert.Contains(t, script, "set -xe") // verbose mode on
	assert.Contains(t, script, "zig build --summary all -Doptimize=ReleaseSafe --verbose")
}

func TestBuildScript_WithoutCacheDir(t *testing.T) {
	bs := NewBuildScript("/src", "", "", false, "")
...
func TestBuildScript_WithOptimization(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: "zig build --summary all -Doptimize=ReleaseSafe",
		},
		{
			name:     "ReleaseFast",
			optimize: "ReleaseFast",
			expected: "zig build --summary all -Doptimize=ReleaseFast",
		},
		{
			name:     "ReleaseSmall",
			optimize: "ReleaseSmall",
			expected: "zig build --summary all -Doptimize=ReleaseSmall",
		},
		{
			name:     "Debug",
			optimize: "Debug",
			expected: "zig build --summary all -Doptimize=Debug",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			bs := NewBuildScript("/src", tt.optimize, "", false, "")
			script := bs.Script()
...
func TestBuildScript_WithTarget(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: "zig build --summary all -Dtarget=x86_64-linux-musl",
		},
		{
			name:     "aarch64-linux-gnu",
			target:   "aarch64-linux-gnu",
			expected: "zig build --summary all -Dtarget=aarch64-linux-gnu",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			bs := NewBuildScript("/src", "", tt.target, false, "")
			script := bs.Script()
			assert.Contains(t, script, tt.expected)
		})
	}
...
func TestBuildScript_WithOptimizeAndTarget(t *testing.T) {
	bs := NewBuildScript("/src", "ReleaseFast", "x86_64-linux-musl", false, "")

	script := bs.Script()

	assert.Contains(t, script, "zig build --summary all -Doptimize=ReleaseFast -Dtarget=x86_64-linux-musl")
}
...
func TestBuildScript_WithBuildZon(t *testing.T) {
	...
	bs := NewBuildScript(tempDir, "", "", false, "")
