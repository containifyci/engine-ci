...
func GoImage() string {
	...
	return fmt.Sprintf("%s/%s/%s:%s", container.GetBuild().Registry, "containifyci", "golang-1.22.4", tag)
...
func (c *GoContainer) Prod() error {
	...
	err = c.Container.Push(imageId, fmt.Sprintf("%s/%s/%s:%s", container.GetBuild().Registry, container.GetBuild().Repository, c.Image, c.ImageTag), container.PushOption{Remove: false})
	if err != nil {
