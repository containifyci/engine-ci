...
func testBuilderIntegration(t *testing.T) {
	...
	t.Run("ConfigurationPerformance", func(t *testing.T) {
		...
		assert.Less(t, duration, 50*time.Millisecond, "GetDefaultConfig should be fast")
		...
		duration = time.Since(start)
		assert.Less(t, duration, 100*time.Millisecond, "ValidateConfig should be fast")
		...
		duration = time.Since(start)
		assert.Less(t, duration, 100*time.Millisecond, "LoadFromEnvironmentVariables should be fast")
	})
}
...
func TestConfigurationMemoryUsage(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		// Apply some environment variables
		os.Setenv("ENGINE_CI_LANGUAGE_GO_VERSION", "1.25."+string(rune(i%10)))
	...
	for i, config := range configs {
		assert.Equal(t, "1.25."+string(rune(i%10)), config.Language.Go.Version)
	}
